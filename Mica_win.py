...
#-----------------------------------------------
    ...
    nas = lb(chosen_language(jezik)[0], 40, RAISED, 5)   #Napravljen i dekorisan naslov
    
    con = lb(chosen_language(jezik)[2], 20, RAISED, 5)   #Kreirano dugme za promene kontole
    
    col = lb(chosen_language(jezik)[3], 20, RAISED, 5)   #Kreirano dugme za promenu boje igraca
    
    cr = lb(chosen_language(jezik)[1], 10, FLAT, 5)      # Informacija na dnu stranice ko je napravio igricu

#-----------------------------------------------

    nas.create_label().pack(pady=50)                     # Dugme je ubaceno u prozor
    con.create_label().pack(pady=10)
    col.create_label().pack(pady=10)
    start_game().pack(pady=20)
    cr.create_label().pack(side="bottom")                # Informacija na dnu je ubacena u prozor